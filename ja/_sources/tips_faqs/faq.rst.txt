...
- クライアントライブラリが時々 RPC タイムアウトエラーを投げる (サーバからクライアントが自動切断される)
 ...
 Jubatus サーバは、アイドルタイムアウト (コマンドラインパラメタ :option:`jubatus_server -t` で設定される) が経過すると自動的に接続を切断します。
 RPC呼び出しを再度実行することで接続が再確立されます。サーバによる接続自動切断に起因するタイムアウトを含むRPCエラーのハンドリングについては :doc:`faq_rpc_err_workaround` 参照してください。

 この自動切断機能を無効にするには、 :option:`jubatus_server -t` を 0 (タイムアウトなし) に設定します。
...
- jubaanomalyのスコアが極端に大きな値を返却する

 ある一点の近傍に多数の点が集中しすぎると、 ``ignore_kth_same_point`` のオプションを付与しても、
 anomalyのスコアが極端に大きくなる場合があります。
 これは、floatでの数値計算の丸め誤差により起こる問題です。

 Jubatus 1.0.2以前は、0除算が発生するため ``inf`` を返却するようになっていましたが、
 1.0.3以降では問題の切り分けを行いやすくするため、
 ``inf`` を避け、大きな(10e9-10e10程度のオーダーの)値を返却するように設計しています。
...
- このような速度低下を回避するにはどうすればよいか
 ...
 - 1: ``unlearner`` を利用する
  ...
  バージョン 0.6.0 から、anomalyにて ``unlearner`` が利用できるようになりました。
