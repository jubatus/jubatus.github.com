...
``python`` コマンドが利用できない環境 (Ubuntu 16.04 など) でビルドを行う場合は、 ``sudo apt-get install python2.7`` を実行して ``python`` をインストールするか、 ``waf`` を ``python3`` 経由で (例えば ``python3 ./waf configure``) 実行してみてください。
