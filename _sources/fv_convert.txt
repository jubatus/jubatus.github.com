...
実際にアプリケーションろ書くときに、より高い精度の学習結果を求める場合は、設定をチューニングすることで望ましい結果を得られる可能性がある。
...
例えば、"message"内の文字列からHTMLタグを取り除くためには、
...
各規則は"key", "type", "suffix"の要素からなる辞書を指定する。
 ...
 ========  ==============================================================================================
  ...
  suffix    変換後の結果を格納するkeyのサフィックスを指定する。例えばsuffixに"-detaggeed"が指定され、"name"という名前のkeyに規則が適用された場合、結果は"name-detagged"という名前のkeyに格納される。
 ========  ==============================================================================================
...
.. describe:: libux_splitter.so
 ...
 string_typesで指定できる。ux-trieを利用して、与えられた文書から辞書マッチするキーワードを抜き出して、それぞれを特徴量として利用する。--enable-uxオプション付きでコンパイルすると、/usr/local/lib以下に作成される。
...
.. code-block:: js
      ...
      "string_rules":
      [
        { "key": "message", "type": "mecab", "sample_weight": "bin", "global_weight": "bin" }
      ]
