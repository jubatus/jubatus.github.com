...
.. option:: -z <list>, --zookeeper <list>
   ...
   ``<list>`` is a comma-separated list of ``host:port`` of ZooKeeper servers (e.g., ``10.0.0.1:2181,10.0.0.2:2181,10.0.0.3:2181``).
...
.. option:: -i <count>, --interval_count <count>
   ...
   The update counter is incremented when API that updates the training model (such as ``train`` in the classifier) is called.
...
``jenerator`` generates C++ source of keeper and server skeleton from extended MessagePack-IDL file.
